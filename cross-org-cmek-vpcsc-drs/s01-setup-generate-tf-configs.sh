...
generate_config () {
  ...
  s/{{CUST_PROJECT_ID_SEED}}/$CUST_PROJECT_ID_SEED/;
  s/{{CUST_ADMIN_USER}}/$CUST_ADMIN_USER/;
  s/{{CUST_STATE_BUCKET}}/$CUST_STATE_BUCKET/;
  s/{{CUST_ORG_ID}}/$CUST_ORG_ID/;
  s/{{CUST_ORG_NAME}}/$CUST_ORG_NAME/;
