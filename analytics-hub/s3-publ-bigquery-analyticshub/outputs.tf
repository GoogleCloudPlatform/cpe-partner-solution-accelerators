...
bq --project_id ${data.google_project.publ_bq_fed_ds.project_id} load --source_format NEWLINE_DELIMITED_JSON ${google_bigquery_dataset.fed_src_dataset.dataset_id}.${google_bigquery_table.fed_src_table.table_id} gs://${var.publ_tf_state_bucket}/${google_storage_bucket_object.src_table_data_jsonl.name}
