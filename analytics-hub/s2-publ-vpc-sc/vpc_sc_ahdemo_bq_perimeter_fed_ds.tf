...
resource "google_access_context_manager_service_perimeter" "publ_bq_fed_ds" {
  title                     = "ahdemo_${var.name_suffix}_publ_bq_fed_ds"
  description               = "ahdemo_${var.name_suffix}_publ_bq_fed_ds"
  name                      = "accessPolicies/${google_access_context_manager_access_policy.access_policy.id}/servicePerimeters/ahdemo_${var.name_suffix}_publ_bq_fed_ds"
  ...
  status {
      access_levels       = []
      resources           = var.vpc_sc_dry_run ? [] : [ "projects/${data.google_project.publ_bq_fed_ds.number}" ]
...
resource "google_access_context_manager_service_perimeter_ingress_policy" "publ_bq_fed_ds_ingress_policy_0" {
  perimeter = "${google_access_context_manager_service_perimeter.publ_bq_fed_ds.name}"
  ...
  ingress_to {
      resources = [
          "*",
#          "projects/${data.google_project.pubpubl_bq_fed_dsl_ah_exchg.number}",
      ]
      roles     = []
...
resource "google_access_context_manager_service_perimeter_ingress_policy" "publ_bq_fed_ds_ingress_policy_1" {
  perimeter = "${google_access_context_manager_service_perimeter.publ_bq_fed_ds.name}"
...
resource "google_access_context_manager_service_perimeter_ingress_policy" "publ_bq_fed_ds_ingress_policy_2" {
  perimeter = "${google_access_context_manager_service_perimeter.publ_bq_fed_ds.name}"
...
# CHAINED VIEWS: required for creating the view from shared_ds to the view in src_ds that is querying the view in fed_ds (so shared_ds -> src_ds -> fed_ds)
resource "google_access_context_manager_service_perimeter_egress_policy" "publ_bq_fed_ds_egress_policy_1" {
  perimeter = "${google_access_context_manager_service_perimeter.publ_bq_fed_ds.name}"
...
resource "google_access_context_manager_service_perimeter_egress_policy" "publ_bq_fed_ds_egress_policy_2" {
  perimeter = "${google_access_context_manager_service_perimeter.publ_bq_fed_ds.name}"
