...
#          "projects/${data.google_project.bq_and_ah.number}",
  ...
  egress_policies_bq_and_ah_perimeter = [
    ...
    # Allow egress to bq_src_ds (Google Service -> Google Service) - required for creating the view from bq_and_ah to src_ds
    {
      "from" = {
        "identities" = var.vpc_sc_access_level_corp_allowed_identities
        "identity_type" = null
...
module "regular_service_perimeter_bq_and_ah" {
  ...
  perimeter_name = "ahdemo_${var.name_suffix}_bq_and_ah_perimeter"
  description    = "ahdemo_${var.name_suffix}_bq_and_ah_perimeter"
  ...
  resources = var.vpc_sc_dry_run ? [] : [ data.google_project.bq_and_ah.number ]
  resources_dry_run = var.vpc_sc_dry_run ? [ data.google_project.bq_and_ah.number ] : []
