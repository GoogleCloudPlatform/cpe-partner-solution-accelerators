...
#          "projects/${data.google_project.bq_shared_ds.number}",
  ...
  egress_policies_bq_perimeter_shared_ds = [
    # Allow egress to ah_exchg,nonvpcsc_ah_exchg (Google Service -> Google Service) - required for creating the listing
    {
      "from" = {
        "identities" = var.vpc_sc_access_level_corp_allowed_identities
      }
    ...
    # Allow egress to bq_src_ds (Google Service -> Google Service) - required for creating the view from shared_ds to src_ds
    {
      "from" = {
        "identities" = var.vpc_sc_access_level_corp_allowed_identities
      }
...
module "regular_service_perimeter_bq_shared_ds" {
  ...
  perimeter_name = "ahdemo_${var.name_suffix}_bq_perimeter_shared_ds"
  description    = "ahdemo_${var.name_suffix}_bq_perimeter_shared_ds"
  ...
  resources = var.vpc_sc_dry_run ? [] : [ data.google_project.bq_shared_ds.number ]
  resources_dry_run = var.vpc_sc_dry_run ? [ data.google_project.bq_shared_ds.number ] : []
