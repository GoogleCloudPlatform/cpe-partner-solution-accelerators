  ...
  ingress_policies_bq_perimeter_src_ds = [
    # Allow off-perimeter internal users (Cloud Console users) in var.vpc_sc_access_level_corp_allowed_identities from the corporate network IP ranges
    {
      ...
      "to" = {
        "resources" = [
          "*",
#          "projects/${data.google_project.bq_src_ds.number}",
        ]
        "operations" = {
    ...
    # Allow off-perimeter subscribers (Cloud Console users) in var.vpc_sc_ah_subscriber_identities from anywhere - required for querying src_ds from the view in shared_ds
    {
      "from" = {
        "sources" = {
...
#          "projects/${data.google_project.ah_exchg.number}",
  ...
  egress_policies_bq_perimeter_src_ds = [
    # Allow egress to bq_shared_ds and bq_and_ah (Google Service -> Google Service) - required for creating the view from shared_ds/bq_and_ah to src_ds
    {
      "from" = {
        "identities" = var.vpc_sc_access_level_corp_allowed_identities
      }
    ...
    # Allow egress to #subscriber_project_number (Google Service -> Google Service) - required for querying src_ds from the view in shared_ds
    {
      "from" = {
        "identities" = var.vpc_sc_ah_subscriber_identities
      }
...
module "regular_service_perimeter_bq_src_ds" {
  ...
  perimeter_name = "ahdemo_${var.name_suffix}_bq_perimeter_src_ds"
  description    = "ahdemo_${var.name_suffix}_bq_perimeter_src_ds"
  ...
  resources = var.vpc_sc_dry_run ? [] : [ data.google_project.bq_src_ds.number ]
  resources_dry_run = var.vpc_sc_dry_run ? [ data.google_project.bq_src_ds.number ] : []
