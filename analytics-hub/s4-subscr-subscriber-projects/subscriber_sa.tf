...
resource "google_service_account_iam_member" "subscriber_sa_user" {
  for_each = toset(concat(var.subscr_subscriber_sa_users_iam_members, [ "serviceAccount:${data.google_compute_default_service_account.default_xpn.email}", "serviceAccount:${data.google_compute_default_service_account.default_vm.email}", "serviceAccount:${var.subscr_terraform_sa_email}" ]) )
