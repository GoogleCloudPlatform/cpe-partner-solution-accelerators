...
resource "google_compute_address" "vm_ip_admin" {
  for_each                           = var.admin_vms
...
resource "google_compute_instance" "vm_admin" {
  for_each                           = var.admin_vms
  ...
  network_interface {
    ...
    network_ip         = google_compute_address.vm_ip_admin[each.key].address
  }
